...
import { Home as HomeIcon, Grid, ShoppingBag, Truck, Search, ArrowLeft } from 'lucide-react';
...
const HeeraLayout: React.FC<HeeraLayoutProps> = ({ children }) => {
  ...
  // Hide global header on specific pages that have custom headers
  const hideGlobalHeader = location.pathname === '/EC' ||
  ...
  const navItems = [
    { icon: HomeIcon, label: 'হোম', path: '/EC' },
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950 font-bangla text-zinc-900 dark:text-zinc-100 flex flex-col">
        ...
        <header className="sticky top-0 z-50 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800">
          <div className="max-w-md mx-auto px-4 h-16 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <Link to="/EC" className="text-lg font-bold tracking-tighter text-zinc-900 dark:text-white flex items-center gap-2">
                হীরা স্টোর
              </Link>
            </div>
      ...
      {/* Main Content */}
      <main className={cn("flex-1 max-w-md mx-auto w-full pb-24")}>
        {children}
      </main>
